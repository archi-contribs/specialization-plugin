...
public class SpecializationSwitchLabelsHandler extends AbstractHandler {
    private static final SpecializationLogger logger = new SpecializationLogger(SpecializationSwitchLabelsHandler.class);
    ...
    public Object execute(ExecutionEvent event) throws ExecutionException {
        ...
        if ( selectedObject != null ) {
            ...
            if ( selectedObject != null ) {
                ...
                for ( IProperty prop: selectedView.getProperties() ) {
                    if ( SpecializationPlugin.areEqual(prop.getKey(), "replace labels") ) {
                        if ( SpecializationPlugin.areEqual(prop.getValue(), "true") ) {
                            logger.trace("Switching off customized labels");
                            prop.setValue("false");
                        ...
                        logger.trace("Switching on customized labels");
                        prop.setValue("true");
                        SpecializationRefreshViewHandler refreshHandler = new SpecializationRefreshViewHandler();
                        refreshHandler.execute(event);
                        refreshHandler = null;
                ...
                // if we're here, this means that there is no "replace labels" property, so we should create one
                IProperty prop = IArchimateFactory.eINSTANCE.createProperty();
                prop.setKey("replace labels");
                prop.setValue("true");
